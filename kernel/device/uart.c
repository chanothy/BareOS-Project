...
{
  while ((uart[UART0_STAT_REG] & UART_IDLE) == 0)
...
char __sys_getc(void)
{
  ...
  while ((ch = readchar) == NULL)
    ;                            /*  a value to be placed  into 'readchar'      */
...
void uart_handler(void)
{
  ...
  {                                /*  If interrupt was caused by a keypress                                                  */
  ...
  { /*  If interrupt was caused by UART awaiting char                                          */
    if (tty_out_count > 0)
    {
      char toSend = tty_out[tty_out_head];
      uart[UART0_RW_REG] = toSend;
